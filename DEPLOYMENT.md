...
1. **Создайте файл с паролем** (один раз перед первым запуском или после обновления, если добавили защиту):
   ```bash
   cp nginx/htpasswd.example nginx/htpasswd
   ```
   По умолчанию в примере: логин **admin**, пароль **admin**. Обязательно смените в продакшене.
